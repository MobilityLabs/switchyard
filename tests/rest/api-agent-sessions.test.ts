...
describe("PATCH /agent-sessions/:id", () => {
  it("ends the session with an exit code", async () => {
    ...
    expect(res.status).toBe(200);
    expect(await body<Record<string, unknown>>(res)).toMatchObject({
      status: "exited",
      exitCode: 0,
    });
  });
  ...
  it("reports the non-numeric id verbatim instead of NaN (SYD-105)", async () => {
    ...
    expect(res.status).toBe(400);
    expect(await body<{ error: string }>(res)).toEqual({
      error: "Agent session abc does not exist.",
    });
  });
