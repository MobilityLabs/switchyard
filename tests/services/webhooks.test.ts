...
import { addWebhook, listWebhooks, removeWebhook, setWebhookActive } from "../../src/services/webhooks.js";
...
describe("webhooks", () => {
  it("registers, lists, scopes to a project, and removes", () => {
    ...
    const scoped = addWebhook(db, human, { url: "http://example.com/syd", projectKey: "SYD", secret: "s3cret" });
    ...
    expect(() => addWebhook(db, human, { url: "http://x.com", projectKey: "NOPE" })).toThrowError(/no project with key/i);
  });
  ...
  it("rejects agent actors managing webhooks", () => {
    ...
    expect(() => addWebhook(db, agent, { url: "http://example.com/other" })).toThrowError(/only humans manage webhooks/i);
    expect(() => removeWebhook(db, agent, hook.id)).toThrowError(/only humans manage webhooks/i);
    expect(() => setWebhookActive(db, agent, hook.id, false)).toThrowError(/only humans manage webhooks/i);
  });
});
