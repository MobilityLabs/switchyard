...
**Engines (SYD-186/SYD-187):** `engine: "claude"` (default) or `"codex"` on `WorkerConfig` — one worker process per engine, each with its own token + poll label (`auto` / `auto-codex`). Codex (`Dockerfile.worker.codex`, `scripts/container-entry.codex.sh`, `scripts/engines/codex.ts`) runs `codex exec --dangerously-bypass-approvals-and-sandbox` with the switchyard MCP via `$CODEX_HOME/config.toml` (`bearer_token_env_var`); auth is the user's ChatGPT login injected at `chatgpt.com` by the sidecar — the container gets a placeholder `auth.json` (real non-secret `CODEX_ACCOUNT_ID`, placeholder token) + the CA via `SSL_CERT_FILE`, never the real `CODEX_OAUTH_TOKEN` (held only in the sidecar). `codex` requires proxy egress (rejected with `egress: "open"`). Build: `npm run build:worker-image-codex`.
