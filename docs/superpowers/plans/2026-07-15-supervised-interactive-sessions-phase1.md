...
**Absorption trade — `addComment` is a second status-write path outside the divert.** `addComment` (`src/services/comments.ts:47-59`) writes `issues.status` directly via `tx.update(issues)` when a human clears `needsInput` on an escalated issue — it does not go through `updateIssue`, so it never sees the hard-gate divert. Under full absorption, `actor.type === "human"` passes for the supervised principal's `actor`, so a supervised session can call `request_human_input` and then answer its own escalation, writing the issue to `todo` and releasing its own claim (`assigneeId: null`) without the gate ever seeing it. This is **bounded today, not fixed**: only `done` is gateable (the `supervised.hard_gate_actions` setting validator in Task 5 rejects any other value — see Task 5, Step 3), and `todo` is not a gated transition, so this path cannot currently reach a hard-gated status. It would start to matter if a future setting change gated `todo` (or any other status `addComment` can write), or if another absorption-passing direct `tx.update(issues)` write path were added elsewhere. See Deferred.
...
- **Route `addComment`'s status write through the gate:** its `needsInput`-clear-and-release path (`src/services/comments.ts:47-59`) writes `issues.status` directly, bypassing `updateIssue`'s hard-gate divert (see the absorption trade in the threat-boundary section above). Harmless today because only `done` is gateable and this path only ever writes `todo`, but the durable fix is to route this write through `updateIssue` (or centralize all status writes through one gated funnel) so a future gated status can't be reached around the gate.
