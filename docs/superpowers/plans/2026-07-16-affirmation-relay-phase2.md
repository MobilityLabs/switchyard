 ...
 *
 * There is no custom crypto here, and that is the entire point of choosing
 * ssh-keygen over a WebAuthn relying-party implementation (research §4.4): the
 * UV-bit check that makes this gate meaningful is enforced by ssh-keygen
 * against the `verify-required` option in allowed_signers, not by us.
 *
